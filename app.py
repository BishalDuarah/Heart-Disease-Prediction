...
def get_binary_file_downloader_html(df):
    ...
    href = f'<a href="data:application/octet-stream;base64,{b64}" download="predictions.csv">Download Predictions CSV</a>'
    return href

st.title("Guardian of Heart") #Gar-Di-An
...
with tab1:
    ...
    # Try different possible column name formats to match your trained models
    ...
    def predict_heart_disease(data):
        predictions = []  # Move this inside the function and create fresh each time
        for modelname in modelnames:
            try:
                model = pickle.load(open(modelname, 'rb'))
                
                # Ensure data matches model's expected features
                if hasattr(model, 'feature_names_in_'):
                    expected_features = model.feature_names_in_
                    # Reorder data to match expected features
                    data_ordered = data.reindex(columns=expected_features, fill_value=0)
                    prediction = model.predict(data_ordered)
                else:
                    prediction = model.predict(data)
                    
                predictions.append(prediction[0])  # Get the single prediction value
                
            except Exception as e:
                st.error(f"Error with model {modelname}: {str(e)}")
    ...
    if st.button("Submit"):
        ...
        for i in range(len(result)):
            ...
            if result[i] is None:
                st.write("Error occurred with this model")
            elif result[i] == 0:
...
with tab2:
 ...
 """ )
    ...
    if uploaded_file is not None:
        ...
        expected_columns = ['Age', 'Sex', 'ChestPainType', 'RestingBP', 'Cholesterol','FastingsBS',
                            'RestingsECG', 'MaxHR', 'ExerciseAngina', 'Oldpeak', 'ST_Slope']
